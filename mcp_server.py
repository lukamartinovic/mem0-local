...
TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        "description": "Store text directly as a memory WITHOUT LLM fact extraction or dedup checks. "
                       ...
                       ""
                       "## How to extract facts for agent-managed storage"
                       ""
                       "Follow these rules when extracting facts from content before calling this tool:"
                       ""
                       "1. ONE FACT PER CALL. Each add_raw_memory call should store a single, "
                       "self-contained fact. Do NOT pack multiple facts into one call."
                       ""
                       "2. BE CONCISE. Aim for 10-30 words. The embedding captures meaning better "
                       "from a focused sentence than from a long paragraph."
                       "   GOOD: 'Auth service uses JWT with 24-hour token expiry and 7-day refresh token expiry'"
                       "   BAD:  'The user mentioned authentication, database choice, deployment strategy, and CI/CD pipeline details'"
                       ""
                       "3. BE SPECIFIC. Include names, versions, ports, and concrete values."
                       "   GOOD: 'PostgreSQL 15 runs on port 5432 with pgvector extension for embeddings'"
                       "   BAD:  'They use some database'"
                       ""
                       "4. PRESERVE DECISIONS AND RATIONALE. When someone chose X over Y, store both "
                       "the choice and the reason."
                       "   GOOD: 'Chose PostgreSQL over MongoDB because ACID transactions required for billing service'"
                       ""
                       "5. USE ACTIVE VOICE. 'Auth service uses JWT' not 'JWT is used by the auth service'."
                       ""
                       "6. EXTRACT, DO NOT SUMMARIZE. Store the atomic fact, not a paraphrase of the "
                       "conversation around it."
                       "   GOOD: 'API rate limit is 100 requests per minute per API key'"
                       "   BAD:  'They discussed API limits and decided on something reasonable'"
                       ""
                       "7. SKIP OPINIONS AND FILLER. Do not store greetings, agreement, or conversational "
                       "noise. Only store actionable technical facts."
                       ""
                       "8. SCOPE WITH user_id. Set user_id to the project or team name so searches are "
                       "scoped correctly. Use the same user_id for related facts."
                       ""
                       "Example workflow — given this conversation:"
                       "  'We migrated from REST to GraphQL using Apollo Server because mobile clients "
                       "   were over-fetching. The gateway runs on port 4000. We kept REST for webhooks.'"
                       ""
